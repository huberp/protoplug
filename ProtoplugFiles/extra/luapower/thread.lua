...
function M.init_state(state,arg)
	state:openlibs()
...
function M.new(func, ...)
	...
	M.init_state(state,...)
	--the package.path, packge.cpath and protoplug_dir are not set correctly for use in protoplug.
	--set it according to https://stackoverflow.com/questions/4125971/setting-the-global-lua-path-variable-from-c-c
	
	local replacedPackagePath = package.path:gsub("\\","\\\\")
	print("package.path: " .. replacedPackagePath)
	local do_res,err= state:dostring("package.path = ".."\""..replacedPackagePath.."\"");
	print(": ".. tostring(do_res).."; "..(err or ""))
	
	local replacedPackageCPath = package.cpath:gsub("\\","\\\\") 
	print("package.cpath: " .. replacedPackageCPath)
	local do_res,err= state:dostring("package.cpath = ".."\""..replacedPackageCPath.."\"");
	print("2: ".. tostring(do_res).."; "..(err or ""))
	
	local replacedProtoplugDir = protoplug_dir:gsub("\\","\\\\")
	print("replacedProtoplugDir: " .. replacedProtoplugDir)
	local do_res,err= state:dostring("protoplug_dir = ".."\""..replacedProtoplugDir.."\"");
	print("3: ".. tostring(do_res).."; "..(err or ""))
	
	--protoplug_path
	local replacedProtoplugPath = protoplug_path:gsub("\\","\\\\")
	print("replacedProtoplugPath: " .. replacedProtoplugPath)
	local do_res,err= state:dostring("protoplug_path = ".."\""..replacedProtoplugPath.."\"");
	print("4: ".. tostring(do_res).."; "..(err or ""))
	
	local do_res,err= state:dostring("require \"include/protoplug\"");
	print("5: ".. tostring(do_res).."; "..(err or ""))
	

	-- note: the next block is necessary. it crashed protoplug without it
	state:getglobal("package")
	state:getfield(-1,"path")
	local read_path = state:tostring(-1)
	print("Read Path From State: '"..read_path.."'")
	--ende settingstuff 

...
local function test_pthread_creation()
	...
	local dllname = protoplug_dir.."\\lib\\libwinpthread-1.dll" 
	print(dllname)
	state:openlibs(dllname)
	state:push(protoplug_dir)
	state:setglobal("protoplug_dir")
	state:push(function()
		local ffi = require'ffi'
		local r = protoplug_dir..glue
	    local glue = require(r)
